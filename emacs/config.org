...
=elpaca= is a lightweight package manager for Emacs.  It simplifies the process
of installing, managing, and updating Emacs packages.  With elpaca, you can
easily extend the functionality of your Emacs editor by installing various
packages available in the package archive.
...
for the use-package macro in Elpaca.  This means that whenever a package is
added using Elpaca, it will also configure it to be used with the =use-package=
macro.  This allows for easier and more consistent package configuration in
Emacs.  =(elpaca-wait)= ensures that all asynchronous Elpaca package operations
finish before continuing with the rest of the Emacs configuration.
...
The following function retrieves the OpenAI API key named "openai" from the
macOS keychain.
...
The following code setups ChatGPT for Emacs.  The package =openai= must be
installed and configured before the =chatgpt= package.
...
cross-referencing capabilities.  To ensure proper functionality, it first
installs the =project= package via =elpaca= since =eglot= does not work with the
default built-in =project= library.
...
#+begin_src emacs-lisp
  (defconst jess/c++-lsp-server
    '("clangd"
      "-j 8"                        ;; Number of threads for background indexing
      "--background-index"          ;; Enables background indexing
      "--clang-tidy"                ;; Enables Clang-Tidy static analysis
      "--cross-file-rename"         ;; Enables cross-file renaming of symbols
      "--completion-style=detailed" ;; Configures detailed completion style
      "--pch-storage=memory")       ;; Sets precompiled header storage to memory
    )
#+end_src
...
#+begin_src emacs-lisp
  (use-package pdf-tools
    ...
    (add-hook 'pdf-view-mode-hook 'jess/disable-line-num-and-hl))
#+end_src
