...
These key bindings  provide convenient shortcuts for using =avy=  to navigate to
specific characters or locations in the buffer.
...
This code  snippet configures the  behavior of  the directory listing  in Emacs'
=dired= mode. It sets the =insert-directory-program= variable to the path of the
=ls= command and  customizes the =dired-listing-switches= variable  based on the
value of the =EMACS_LS_OPTS= environment variable, if available.
